...
Ссылка на сайт (https://www.eeeexampe.ru/)
...
Для подключения используется переменная окружения DATABASE_URL, формат:
```bash
DATABASE_URL=postgres://USER:PASSWORD@HOST:PORT/DBNAME
```
### 🌍 Геокодирование с Яндекс.Карт
Чтобы определить координаты адресов доставки и ресторанов, используется **Яндекс Геокодер API**. Для его работы необходим API-ключ.

Добавьте в переменные окружения следующее:
...
2. Найдите Яндекс Геокодер

3. Создайте новый API-ключ

4. Скопируйте ключ и добавьте его в файл .env или окружение сервера
